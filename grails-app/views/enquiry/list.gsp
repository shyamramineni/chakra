				...
				<g:each in="${enquiryInstanceList}" status="i" var="enquiryInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>
						    ...
						    <g:link action="quotation" controller="quotation" params="['enquiry.id': enquiryInstance.id, 'customer.id': enquiryInstance?.customer.id]">Create Quotation</g:link><br>
						    </g:if>
						    <g:if test ="${enquiryInstance.status != 'MANUAL QUOTATION'}">
						    ...
						    <g:link class="editUser" action="editUser" id="${enquiryInstance?.id}">Reassign User</g:link><br>
						    </g:if>
						    <g:if test ="${enquiryInstance.status == 'REQUEST REASSIGN'}">
						    <g:link action="denyRequest" controller="enquiry" params="['enquiry.id': enquiryInstance.id, 'assigned.id': enquiryInstance?.assigned.id]">Deny Request</g:link>
						    </g:if>
						    </g:if>
						    </g:if>
						</td>
