...
function update_mesh_item_row_price(jqRowTr)
{
	...
	var response;
	var itemid = $(jqRowTr).children(".header_pid").attr('name');
	var request = "wirebrand="+wirebrand+"&itemid="+itemid+"&requestType=MeshItemCostRequest";
	$.ajax({
        type:'POST',
        url:"/chakrap/sheetSpecification/sheetSpecPrice",
        data: request,
	...
	$(jqRowTr).children(".header_price").text(parseFloat(response.responseData.price).toFixed(2));
}
...
function mesh_item_wirebrand_change(e)
{
	...
	if(atcg_wirebrand_first_selection_flag)
	{
		$("#add_to_cart_grid").children(".item_entry_box").children(".item_entry_table").find("tr").each(function(index, element) {
			console.log("wirebrand change:"+val);
			console.log(element);
...
function selected_item_click(e)
{
	...
	if(type == "mesh_item_tree")
	{
		$.ajax({
	        type:'POST',
	        url:"/chakrap/cageDesign/getSheetSpecifications",
	...
	else if(type =="cage_item_tree")
	{
		$.ajax({
	        type:'POST',
	        url:"/chakrap/cagePartType/getCageParts",
	        data: "id="+id+"&requestType=AddToGridBoxItemsRequest",
	        success: function(data, textStatus, jqXHR) {
	        	console.log(jqXHR.responseText);
	            response = jQuery.parseJSON(jqXHR.responseText);
	...
	/*$.post("/index.html",request,function(data, textStatus, jqXHR) {
		response = jQuery.parseJSON(jqXHR.reponseText);
	});*/
	...
	if(type == "mesh_item_tree")
		...
		itemsArray = response.responseData.cageItems;
	console.log(itemsArray);
	for(var index in itemsArray)
	{
		console.log(element);
		var element = itemsArray[index];
		var backend_id = element.id;
		var mid = element.itemId;
		...
		if(type == "mesh_item_tree")
		{
			entry = $('<tr class="mesh_item_selection_entry" name="'+type+'">\
                            <td class="header_pid" name="'+backend_id+'">'+mid+'</td>\
	...
	$("#add_to_cart_grid").children(".item_entry_box").children(".item_entry_table").find("tr").each(function(index, element) {
		...
		if(qty != 0)
		{
			...
			if(shopping_cart_summary[mid] == undefined)
				...
				old_cnt++;
			var obj = new Object();
			obj.cagePartSpecId = tmpdata.cagePartSpecId;
			obj.id = mid;
			obj.backend_id = $(element).children("td[class=header_pid]").attr('name');
...
function submit_shopping_cart_click(e)
{
	...
	var meshItems = {items:[]};
	var cageParts = {items:[]};
	var cagePartSpecs = new Array();
	for(var index in shopping_cart_summary)
	{
		var element = shopping_cart_summary[index];
		if(element!=null)
		{
			if(element.type == "mesh_item_tree")
			{
				//meshItems += '{"id":"'+element.backend_id+'","qty":"'+element.qty+'","superid":"'+superid_tracker[element.backend_id]+'","wirebrand":"'+element.wirebrand+'", "cagePartSpecId":"'+element.cagePartSpecId+'"},';
				meshItems.items.push({
					id: element.backend_id,
					qty: element.qty,
					cagePartSpec: superid_tracker[element.backend_id],
					wirebrand: element.wirebrand
				});
			}
			else if(element.type == "cage_item_tree")
			{
				//cageParts += '{"id":"'+element.backend_id+'","qty":"'+element.qty+'","superid":"'+superid_tracker[element.backend_id]+'", "cagePartSpecId":"'+element.cagePartSpecId+'"},';
				if(!cagePartSpecs[superid_tracker[element.backend_id]])
					cagePartSpecs[superid_tracker[element.backend_id]] = {cagePartSpec: superid_tracker[element.backend_id], cageParts: []};
				cagePartSpecs[superid_tracker[element.backend_id]].cageParts.push({
					id: element.backend_id,
					qty: element.qty,
					cagePartSpec: superid_tracker[element.backend_id],
					wirebrand: element.wirebrand
	...
	for(i in cagePartSpecs) {
		cageParts.items.push(cagePartSpecs[i]);
	}
	meshItems.items.push({id: ""});
	cageParts.items.push({cagePartSpec: ""});
	//meshItems += '{"id":""}]}';
	//cageParts += '{"id":""}]}';
	$("#mesh_items_field").val(JSON.stringify(meshItems));
	$("#cage_parts_field").val(JSON.stringify(cageParts));
	/*var item_pairs = null;
	for( var i in shopping_cart_summary )
	{
		if(item_pairs == null)
			...
			item_pairs += ", ";
		item_pairs+='{"id":"'+shopping_cart_summary[i].backend_id+'", "qty":"'+shopping_cart_summary[i].qty+'"}\n';
	}
