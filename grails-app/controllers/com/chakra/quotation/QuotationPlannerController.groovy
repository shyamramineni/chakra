...
class QuotationPlannerController {
	...
	def quotationPlannerSubmit(){
		...
		notificationService.addNotification(DummyUser.findByUsername('admin')?.toString(),Notification.NOTIFMSG_QUOTATION_GENERATED, true, ""+g.link(action:"show", controller:"quotation", id: quotationInstance.id) { quotationInstance.quotationId }, Notification.NOTIFTYPE_APP)
		notificationService.addSmsNotification(DummyUser.findByUsername('admin')?.toString(),"Quotation of value "+quotationModel.totalCost+" generated by "+DummyUser.findByUsername('admin')?.toString()+" .", true, Notification.NOTIFTYPE_SMS, quotationInstance.customer.mobileNumber)
