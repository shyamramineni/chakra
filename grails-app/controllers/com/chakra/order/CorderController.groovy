...
class CorderController {
	...
	def save() {
        ...
        if (!corderInstance.save(flush: true)) {
			...
			corderInstance.comments = params.comments+" "+"Created By "+new DummyUser(username: 'dummy').toString()+new Date().format("dd/MM/yy-hh:mm")+"</br>"
			if (!quotationInstance.save(flush: true))  {
				render(view: "create", model: [corderInstance: corderInstance])
				return
			}
			notificationService.addSmsNotification(new DummyUser(username: 'dummy').toString(), Notification.NOTIFMSG_SMS_ORDER_CONFIRM+corderInstance.orderId+", DATE OF DELIVERY: "+corderInstance.deliveryDate.format("dd/MM/yyyy")+", BALANCE TO BE PAID Rs."+corderInstance.balance, true, Notification.NOTIFTYPE_SMS, corderInstance.customer.mobileNumber)
			notificationService.addNotification(new DummyUser(username: 'dummy').toString(), Notification.NOTIFMSG_REALIZED, true, ""+g.link(action:"show", controller:"quotation", id: quotationInstance.id) { corderInstance.orderId+"-"+quotationInstance.quotationId }, Notification.NOTIFTYPE_APP)
			
		}
		if(corderInstance.quotation?.enquiry?.enquiryId != null) {
			notificationService.addNotification(new DummyUser(username: 'dummy').toString(),Notification.NOTIFMSG_ORDER_CREATED, true, ""+g.link(action:"show", controller:"corder", id: corderInstance.id) { corderInstance.orderId+"-"+corderInstance.quotation.quotationId+"-"+corderInstance.quotation?.enquiry?.enquiryId }, Notification.NOTIFTYPE_APP)
		} else {
			notificationService.addNotification(new DummyUser(username: 'dummy').toString(),Notification.NOTIFMSG_ORDER_CREATED, true, ""+g.link(action:"show", controller:"corder", id: corderInstance.id) { corderInstance.orderId+"-"+corderInstance.quotation.quotationId+"- From Planner" }, Notification.NOTIFTYPE_APP)
		}
	...
	def corderBySales() {
		...
		DummyUser user = new DummyUser(username: 'dummy')
		log.debug'user '+user
	...
	def blockStock() {
		...
		if(!stockAvailable) {
			...
			NotificationService notificationService = new NotificationService()
			notificationService.addNotification(new DummyUser(username: 'dummy').toString(), Notification.NOTIFMSG_BLOCKSTOCK, true, ""+g.link(action:"show", controller:"corder", id: corder.id) { corder.orderId }, Notification.NOTIFTYPE_APP)
	...
	def update() {
		...
		corderMeshPartList.each { sheetSpecFor ->
            ...
            def wireBrandInstance = sheetSpecFor.wireBrand
		...
		corderInstance.comments = params.comments+" "+"Edited By "+new DummyUser(username: 'dummy').toString()+new Date().format("ddMMyyhhmm")+"</br>"
