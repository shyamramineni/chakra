...
<html>
	...
	<body>
		...
		<div id="list-enquiry" class="content scaffold-list" role="main">
			...
			<table class="table table-bordered table-striped table-condensed">
				...
				<g:each in="${enquiryInstanceList}" status="i" var="enquiryInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="userShow" id="${enquiryInstance.id}">${fieldValue(bean: enquiryInstance, field: "enquiryId")}</g:link></td>
						...
						<td>
							...
							<g:link action="acceptEquiry" controller="enquiry" params="['enquiry.id': enquiryInstance?.id,'assigned.id': enquiryInstance?.assigned?.id]">Accept</g:link>/<g:link action="requestReassign" controller="enquiry" params="['enquiry.id': enquiryInstance?.id,'assigned.id': enquiryInstance?.assigned?.id]">Request Reassign</g:link><br>
							</g:if>
							<sec:ifAnyGranted roles="ROLE_ADMIN,ROLE_SALES"><g:link action="manualQuotation" controller="enquiry" params="['enquiry.id': enquiryInstance.id, 'customer.id': enquiryInstance?.customer.id]">Manual Quotation</g:link></sec:ifAnyGranted><br>
						    <g:link action="quotation" controller="quotation" params="['id': enquiryInstance.id, 'customer': enquiryInstance?.customer.id]">Create Quotation</g:link><br>
						    </g:if>				    
			...
			<ul>
			<li><sec:ifAllGranted roles="ROLE_SALES"><g:link class="create" action="create"><g:message code="default.new.label" args="[entityName]" /></g:link></sec:ifAllGranted></li>
			</ul>
