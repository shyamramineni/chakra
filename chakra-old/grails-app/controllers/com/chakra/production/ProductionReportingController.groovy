...
class ProductionReportingController {
	...
	def edit() {
		...
		def stockInstance = productionReportingInstance.stock
		println'stockInstance '+stockInstance
		println productionReportingInstance.stock.stockOrderItems
		if (!stockInstance) {
			flash.message = message(code: 'default.not.found.message', args: [message(code: 'stock.label', default: 'Stock'), params.id])
			redirect(action: "list")
			return
	...
	def update() {
		...
		productionReportingInstance.comments = params.comments+" "+"Edited By "+springSecurityService.currentUser.toString()+new Date().format("dd/MM/yy-hh:mm")+"<br>"
